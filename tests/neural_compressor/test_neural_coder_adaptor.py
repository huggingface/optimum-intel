...
class NeuralCoderAdaptorTest(unittest.TestCase):
    ...
    def test_launcher(self):
        ...
        modular_pattern['pytorch_inc_static_quant_fx'] = \
            NeuralCoderAdaptor.default_quant_static
        modular_pattern['pytorch_inc_dynamic_quant'] = \
            NeuralCoderAdaptor.default_quant_dynamic
        modular_pattern['inc_auto'] = \
            NeuralCoderAdaptor.default_quant_dynamic

        Launcher.execute(args, use_modular=True,
                         modular_pattern=modular_pattern, use_inc=False)

        # determine if the code optimization is correct

        import filecmp
        self.assertEqual(True, filecmp.cmp('run_glue_target.py',
                         'run_glue_source_optimized.py'))
