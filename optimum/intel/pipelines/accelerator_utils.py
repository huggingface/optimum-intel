...
OV_TASKS_MAPPING = {
    ...
    "image-text-to-text": (OVModelForVisualCausalLM, OVModelForMultimodalLM),
    ...
    "text-to-audio": (OVModelForTextToSpeechSeq2Seq, OVModelForMultimodalLM),
...
def get_openvino_model_class(
    ...
    if task == "automatic-speech-recognition":
        ...
        if len(task_classes) > 1 and OVModelForMultimodalLM in task_classes:
            if config is None:
                hub_kwargs = {
                    "trust_remote_code": model_kwargs.pop("trust_remote_code", False),
                    "revision": model_kwargs.pop("revision", None),
...
def patch_pipelines_to_load_accelerator_model(accelerator: str):
    ...
    if accelerator == "openvino":
        ...
        raise ValueError(f"Accelerator '{accelerator}' is not supported. Only 'openvino' is supported.")
