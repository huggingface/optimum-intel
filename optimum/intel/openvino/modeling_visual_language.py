...
class OVLfm2VlLanguageModel(OVModelWithEmbedForCausalLM):
    ...
    def _create_empty_cache(self, config, batch_size):
        ...
        key_cache = [
            np.zeros((batch_size, num_kv_heads, 0, head_dim), dtype=np.float32) for _ in range(num_attn_layers)
        ]
        value_cache = [
            np.zeros((batch_size, num_kv_heads, 0, head_dim), dtype=np.float32) for _ in range(num_attn_layers)
        ]
...
class _OVLfm2VlForCausalLM(OVModelForVisualCausalLM):
    def __init__(
        ...
        **kwargs,
        ...
        self._openvino_config = None
        if quantization_config:
            from optimum.intel.openvino.configuration import OVConfig as _OVConfig

            self._openvino_config = _OVConfig(quantization_config=quantization_config)
        self._set_ov_config_parameters()
