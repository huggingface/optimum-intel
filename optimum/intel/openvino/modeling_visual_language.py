...
class OVModelWithEmbedForCausalLM(OVModelForCausalLM):
        ...
        **kwargs,
        ...
        if "visual_pos_masks" in self.input_names:
            if visual_pos_masks is not None:
                ...
                # Shape must match [batch, seq_len] of inputs_embeds for traced deepstack ops
        ...
        if "deepstack_visual_embeds" in self.input_names:
            if deepstack_visual_embeds is not None:
                ...
                # Visual token count must match batch*seq_len for the traced cumsum/gather ops
                seq_len = inputs_embeds.shape[1] if inputs_embeds is not None else 1
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Only extract hidden states when the parent model is actively collecting them
...
class OVTalkerDecoder(OVModelPart):
    """Stateful decoder wrapper for the Talker model (codec generation)."""
    ...
    def compile(self):
        super().compile()
        # Create an InferRequest for stateful async inference
    ...
    def forward(
        ...
        **kwargs,
        ...
        # CodePredictor models have generation_steps as an input
...
class OVCodePredictorDecoder(OVTalkerDecoder):
    """Stateful decoder wrapper for the CodePredictor model (inherits all behavior from OVTalkerDecoder)."""

    _model_name = "code_predictor"


class OVCode2Wav(OVModelPart):
    """Stateless wrapper for the Code2Wav vocoder model."""
...
if is_transformers_version(">=", "4.57.0.dev0"):
    # Qwen3-Omni dense support requires transformers@3d1a4f5e34753e51cb85052539c6ef10cab9a5c1
...
class _OVQwen3OmniForCausalLM(OVModelForVisualCausalLM):
    ...
    def _all_ov_model_paths(cls) -> Dict[str, str]:
        ...
        paths = parent_fn(cls)
        # Include codec_embedding weights so they're downloaded from Hub alongside OV models
        paths["cp_codec_embedding"] = "code_predictor_codec_embedding.npy"
        return paths
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Load CodePredictor codec_embedding weights for runtime token embedding
        self._cp_codec_embedding = None
        if model_save_dir is not None and self.code_predictor is not None:
            npy_path = Path(str(model_save_dir)) / "code_predictor_codec_embedding.npy"
            if npy_path.exists():
                self._cp_codec_embedding = torch.from_numpy(np.load(str(npy_path)))
            else:
                logger.warning(
    ...
    def _save_pretrained(self, save_directory):
        ...
        if self._cp_codec_embedding is not None:
            np.save(
                os.path.join(save_directory, "code_predictor_codec_embedding.npy"),
                self._cp_codec_embedding.cpu().numpy(),
            )
        ...
        **kwargs,
        ...
        if pixel_values is not None and image_grid_thw is not None:
            ...
            if image_token_id is not None:
                ...
                inputs_embeds = inputs_embeds.masked_scatter(image_mask, image_embeds)
                # Track visual positions and deepstack features for language model injection
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Collect hidden states during generation for the Talker pipeline.
        # Parent class (OVModelWithEmbedForCausalLM) already extracts these from the InferRequest
        # and attaches them as result.last_hidden_state / result.intermediate_hidden_state.
    ...
    def _process_audio_inputs(self, input_features, feature_attention_mask):
        ...
        # Compute cu_seqlens for windowed attention in the audio encoder transformer layers
    ...
    def _get_talker_user_parts(
        ...
        """Build talker input for user segments — text gets text_projection, multimodal gets hidden_projection."""
    ...
    def _get_talker_assistant_parts(
        ...
        """Build talker input for assistant segment — combines text projection with codec special tokens."""
        ...
        # Use the talker text embeddings model for codec token embeddings
    ...
    def _embed_cp_token(self, token_ids: torch.Tensor, step: int) -> torch.Tensor:
        """Embed a token using the CodePredictor's codec_embedding for the given step.

        Args:
            token_ids: shape [batch, 1] — sampled code predictor token IDs
            step: 0-based step index (maps to codec_embedding[step])
        Returns:
            Embedded tensor of shape [batch, 1, hidden_size]
        """
        if self._cp_codec_embedding is not None:
            weights = self._cp_codec_embedding[step]
            return torch.nn.functional.embedding(token_ids, weights)
        # Fallback when weights aren't available (old export without codec_embedding.npy)
        talker_config = getattr(self.config, "talker_config", None)
        cp_config = getattr(talker_config, "code_predictor_config", None) if talker_config else None
        hidden_size = cp_config.hidden_size if cp_config else 64
        return torch.zeros(token_ids.shape[0], 1, hidden_size)
    ...
    def _run_talker_generation(
        ...
        """Run the Talker autoregressive loop with nested CodePredictor calls.

        Matches the HF reference flow:
        1. Talker generates the first codec token per step
        2. CodePredictor generates remaining num_code_groups-1 tokens
        3. Next talker input = sum of all codec representations + trailing text
        """
        talker_config = getattr(self.config, "talker_config", None)
        if talker_config is None:
        ...
        # Prefill the talker with the input embeddings
        self.talker.reset()
        ...
        for step in range(max_new_tokens):
            # Sample first codec token from talker logits
            ...
            # Embed the first code using the talker's codec embedding
            ...
            # Collect all codec representations for summing (HF sums num_code_groups embeddings)
            ...
            if self.code_predictor is not None and num_code_groups > 1:
                ...
                # CodePredictor prefill: [talker_last_hidden, first_code_embed] (2 tokens)
                ...
                for cp_step in range(num_code_groups - 1):
                    ...
                    # Embed the sampled token using step-specific codec_embedding
                    cp_embed = self._embed_cp_token(cp_token.unsqueeze(0), cp_step)
                    codec_hiddens.append(cp_embed)

                    if cp_step < num_code_groups - 2:
                        # Feed embedded token to CodePredictor for next step
                        cp_logits, cp_hidden = self.code_predictor(
                            inputs_embeds=cp_embed,
                            past_key_values=((),),
                            generation_steps=cp_step + 1,
                        )

            all_codec_codes.append(step_codes)

            # Build next talker input: sum all codec representations + trailing text
        ...
        # Stack codes: [batch=1, num_quantizers, seq_len]
    ...
    def generate(self, *args, **kwargs):
        ...
        # Process audio inputs for thinker
        ...
        if not return_audio:
            # Standard text-only generation — return raw result for backward compat
            return super().generate(*args, **kwargs)
        ...
        # Audio generation: need to collect hidden states during thinker generation
        input_ids = kwargs.get("input_ids", args[0] if args else None)
        if input_ids is not None:
            input_ids = input_ids.clone()

        # Extract talker-specific kwargs
        talker_kwargs = {}
        thinker_kwargs = {}
        ...
        # Build multimodal mask
        if hasattr(thinker_result, "sequences"):
            sequences = thinker_result.sequences
        else:
            sequences = thinker_result
        ...
        # Build special token embeds for talker
