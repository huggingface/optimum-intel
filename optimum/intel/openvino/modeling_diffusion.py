...
from transformers import CLIPFeatureExtractor, CLIPTokenizer, PreTrainedTokenizer
...
from .utils import ONNX_WEIGHTS_NAME, OV_TO_NP_TYPE, OV_XML_FILE_NAME, \
    TEXTUAL_INVERSION_NAME, TEXTUAL_INVERSION_NAME_SAFE, TEXTUAL_INVERSION_EMBEDDING_KEY
...
class OVTextualInversionLoaderMixin:
    ...
    def load_textual_inversion(
        ...
        **kwargs,
        ...
        if not hasattr(self, "text_encoder") or not isinstance(self.text_encoder, OVModelTextEncoder):
            raise ValueError(
                f"{self.__class__.__name__} requires `self.text_encoder` of type `OVModelTextEncoder` for calling"
                f" `{self.load_textual_inversion.__name__}`"
            )
