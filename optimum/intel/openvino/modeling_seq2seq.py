...
class OVModelForSeq2SeqLM(OVBaseModelForSeq2SeqLM, GenerationMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ov_encoder_config = {**self.ov_config} if "CACHE_DIR" in self.ov_config.keys() else {**self.ov_config, "CACHE_DIR": str(encoder_cache_dir)}
        ...
        ov_decoder_config = {**self.ov_config} if "CACHE_DIR" in self.ov_config.keys() else {**self.ov_config, "CACHE_DIR": str(decoder_cache_dir)}
        ...
        if self.use_cache:
            ...
            ov_decoder_past_config = {**self.ov_config} if "CACHE_DIR" in self.ov_config.keys() else {**self.ov_config, "CACHE_DIR": str(decoder_past_cache_dir)}
