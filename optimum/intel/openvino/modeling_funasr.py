...
def _get_ov_model_for_funasr_class():
    """Deferred class construction to avoid circular import with modeling_seq2seq."""
    from .modeling_seq2seq import OVModelForSpeechSeq2Seq

    class _OVModelForFunAsr(OVModelForSpeechSeq2Seq):
        """FunASR-specific subclass of OVModelForSpeechSeq2Seq."""
    ...
    return _OVModelForFunAsr


_OVModelForFunAsr = None


def _get_funasr_class():
    """Get the _OVModelForFunAsr class, constructing it on first access."""
    global _OVModelForFunAsr
    if _OVModelForFunAsr is None:
        _OVModelForFunAsr = _get_ov_model_for_funasr_class()
    return _OVModelForFunAsr
