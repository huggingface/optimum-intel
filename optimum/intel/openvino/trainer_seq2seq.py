...
from transformers.onnx import FeaturesManager, OnnxConfig, export
...
from .trainer import OVTrainer, _openvino_version
from .utils import (
    MAX_ONNX_OPSET,
...
class OVSeq2SeqTrainer(OVTrainer):
    def __init__(self, *args, use_cache=True, **kwargs):
        super().__init__(*args, feature="seq2seq-lm", **kwargs)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if self.compression_controller is not None:
            ...
            onnx_config_encoder = EncoderOnnxConfig(self.model.config, task="default")
            onnx_config_decoder = DecoderOnnxConfig(self.model.config, task=self.feature, use_past=False)
            onnx_config_decoder_with_past = DecoderOnnxConfig(self.model.config, task=self.feature, use_past=True)

            if _openvino_version <= version.Version("2022.2.0"):
                if onnx_opset > MAX_ONNX_OPSET_2022_2_0 + 1:
                    if not self.ov_config.save_onnx_model:
                        logger.warning(
                            f"The minimal ONNX opset for the given model architecture is {onnx_opset}. Currently, "
            ...
            max_onnx_opset = min(onnx_opset, MAX_ONNX_OPSET)
            ...
            self.model.disable_dynamic_graph_building()

            encoder = self.model.get_encoder()
            decoder_with_lm_head = _DecoderWithLMhead(self.model)
            export(
                preprocessor=self.tokenizer,
                model=encoder,
                config=onnx_config_encoder,
                opset=opset,
                output=Path(os.path.join(output_dir, ONNX_ENCODER_NAME)),
            )
            export(
                preprocessor=self.tokenizer,
                ...
                opset=opset,
                output=Path(os.path.join(output_dir, ONNX_DECODER_NAME)),
            )
            ...
            if self.use_cache:
                export(
                    ...
                    opset=opset,
                    output=Path(os.path.join(output_dir, ONNX_DECODER_WITH_PAST_NAME)),
                )
                self._save_openvino(
                    os.path.join(output_dir, ONNX_DECODER_WITH_PAST_NAME),
                    os.path.join(output_dir, OV_DECODER_WITH_PAST_NAME),
                )

            self._save_openvino(os.path.join(output_dir, ONNX_ENCODER_NAME), os.path.join(output_dir, OV_ENCODER_NAME))
            self._save_openvino(os.path.join(output_dir, ONNX_DECODER_NAME), os.path.join(output_dir, OV_DECODER_NAME))

            self.model.enable_dynamic_graph_building()

    def _save_openvino(self, input_path, output_path):
