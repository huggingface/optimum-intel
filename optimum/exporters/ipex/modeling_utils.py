...
from transformers.modeling_outputs import BaseModelOutputWithPast, BaseModelOutputWithPastAndCrossAttentions,CausalLMOutputWithPast
...
class _IPEXAttention(nn.Module):
    ...
    @torch.compiler.disable()
    def pageattn(self, attn_output, query, key_cache, value_cache, past_key_value, input_lens):
    ...
    @torch.compiler.disable()
    def varlen(self, query, key, value, attn_output, seq_len_tensor, input_lens):
        varlen_attention(
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_len == torch.tensor(0, dtype=torch.int32):
            # prefill, remove padding
            seq_len_tensor = torch.cat((input_lens.new_tensor([0]), input_lens.cumsum(-1).int()))
            self.varlen(query, key, value, attn_output, seq_len_tensor, input_lens)
            """
            varlen_attention(
                query.contiguous() if query.device.type == "xpu" else query,
                key.contiguous() if key.device.type == "xpu" else key,
            ...
            self.pageattn(attn_output, query, key_cache, value_cache, past_key_value, input_lens)
            # decode
...
class _IPEXLlamaDecoderLayer(nn.Module):
    ...
    @torch.compiler.disable()
    def mha_linear_add(self, hidden_states, residual):
        return self.self_attn.mha_linear_add(hidden_states, residual)
