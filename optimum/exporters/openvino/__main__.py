        ...
        if library_name == "open_clip":
            ...
            if library_name == "transformers":
                ...
                if has_remote_code and trust_remote_code and task == "image-text-to-text":
                    ...
                    # Some remote-code VLMs (e.g. baidu/Unlimited-OCR) register their causal-LM
                    # class only under the ``AutoModel`` auto_map key. text-generation loading uses
                    # ``AutoModelForCausalLM``; load such models directly with ``AutoModel`` instead.
