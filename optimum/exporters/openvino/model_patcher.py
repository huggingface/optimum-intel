        ...
        for idx, layer in enumerate(self._model.model.layers):
            if layer.moe_enabled:
                ...
                afmoe_moe.down_projs = torch.concat(
                    tuple(afmoe_moe.experts[i].down_proj.weight.float().unsqueeze(0) for i in range(num_experts)),
                    dim=0,
                ).transpose(1, 2)
                afmoe_moe.gate_projs = torch.concat(
                    tuple(afmoe_moe.experts[i].gate_proj.weight.float().unsqueeze(0) for i in range(num_experts)),
                    dim=0,
                ).transpose(1, 2)
