...
class LlamaEagle3Model(LlamaEagle3PreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            expanded_attn_mask = AttentionMaskConverter._expand_mask(attention_mask, torch.float32, tgt_len=input_shape[-1]).to(
        ...
        if hasattr(self, "tree_mask") and self.tree_mask is not None:
            ...
            combined_attention_mask[:, :, -tree_shape0:, -tree_shape1:][tree_mask == 0] = torch.finfo(
                torch.float32
            ).min
