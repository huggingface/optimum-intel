...
def eager_mask_without_vmap(*args, **kwargs) -> Optional[torch.Tensor]:
    ...
    # transformers 5.x passes q_length/q_offset instead of cache_position; synthesize it as a tensor
    # so the underlying optimum-onnx helper (which still consumes cache_position) keeps working
    # without freezing dynamic shape info into Python ints during tracing.
    if "cache_position" not in kwargs and "q_length" in kwargs:
        q_length = kwargs.pop("q_length")
        q_offset = kwargs.pop("q_offset", 0)
        device = kwargs.get("device")
        q_arange = torch.arange(q_length, dtype=torch.long, device=device)
        if isinstance(q_offset, torch.Tensor):
            q_offset = q_offset.to(q_arange.device)
        kwargs["cache_position"] = q_arange + q_offset
...
class Qwen2MoEPatcher(OVDecoderModelPatcher):
    def __enter__(self):
        ...
        # For Transformers 5.0+, use batched_mm as our Transformers 4.x patch still uses torch.where
        if is_transformers_version(">=", "5"):
            self._model.set_experts_implementation("batched_mm")
...
class Qwen3MoeModelPatcher(OVDecoderModelPatcher):
    def __enter__(self):
        ...
        # Apply static MoE patching for both Transformers 4.x and 5.0+
        # The qwen3_moe_forward_patched function handles both versions
        if is_transformers_version(">=", "4.53"):
...
class GptOssModelPatcher(OVDecoderModelPatcher):
    def __enter__(self):
        ...
        # For Transformers 5.0+, use batched_mm
        if is_transformers_version(">=", "5"):
            self._model.set_experts_implementation("batched_mm")
