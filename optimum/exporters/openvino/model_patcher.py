...
def _hunyuan_attention_forward(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    from transformers.models.hunyuan_v1_dense.hunyuan_v1_dense import apply_rotary_pos_emb, repeat_kv
    ...
    kv_seq_len = key_states.shape[-2]
    if past_key_value is not None:
        previous_seq_length = past_key_value.get_seq_length(self.layer_idx)
...
class HunyuanModelPatcher(DecoderModelPatcher):
    def __enter__(self):
        super().__enter__()
        for layer in self._model.model.layers:
            layer.self_attn._orig_forward = layer.self_attn.forward
            layer.self_attn.forward = types.MethodType(_hunyuan_attention_forward, layer.self_attn)
