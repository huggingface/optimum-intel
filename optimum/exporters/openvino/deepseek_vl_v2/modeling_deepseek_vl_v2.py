...
class DeepseekVLV2Config(PretrainedConfig):
    ...
    def __init__(
        ...
        # The top-level VLM forward reads ``self.config.use_cache``; mirror the
        # language model's generation cache setting so ``generate`` works even
        # when the checkpoint config does not declare ``use_cache`` explicitly.
        self.use_cache = kwargs.get("use_cache", getattr(self.language_config, "use_cache", True))

    def get_text_config(self, decoder=False, encoder=False, *args, **kwargs):
        # The decoder / text model is the DeepseekV2 language sub-config.
        # Guard for calls during ``super().__init__`` (before it is set).
        return getattr(self, "language_config", self)
...
class DeepseekVLV2ForCausalLM(DeepseekVLV2PreTrainedModel, _GenerationMixin):
    ...
    def __init__(self, config: DeepseekVLV2Config):
        ...
        # Register tied-weight/parallel metadata on the composite top-level model
        # (required by newer transformers `from_pretrained`).
        self.post_init()
    ...
    def prepare_inputs_for_generation(
        ...
        # Delegate to the modern GenerationMixin implementation for correct
        # cache_position / position_ids / inputs_embeds handling, then inject
        # the image inputs only on the first (prefill) step.
        ...
        # Drop generate-only kwargs (e.g. ``tokenizer`` used for stopping
        # criteria) that the base implementation may propagate but ``forward``
        # does not accept.
        import inspect as _inspect

        _forward_params = set(_inspect.signature(self.forward).parameters.keys())
        model_inputs = {k: v for k, v in model_inputs.items() if k in _forward_params}
        # Decide prefill robustly. ``generate`` in transformers >= 5 passes an
        # already-instantiated (empty) ``DynamicCache`` on the first step, so
        # ``past_key_values is None`` is *not* a reliable prefill signal, and it
        # may not populate ``cache_position`` for custom models. Prefer an
        # explicit ``cache_position``; otherwise fall back to the cache length.
        cp = cache_position
        if cp is None:
            cp = model_inputs.get("cache_position", None)
        if cp is not None:
            is_prefill = int(cp[0]) == 0
        elif past_key_values is not None and hasattr(past_key_values, "get_seq_length"):
            is_prefill = int(past_key_values.get_seq_length()) == 0
        else:
        ...
        if is_prefill:
            ...
            # cached decoding: image tokens are already consumed
...
def _safe_register_config(model_type, config_cls):
    try:
        AutoConfig.register(model_type, config_cls)
    except ValueError:
        # already registered (e.g. imported twice in the same process)
        pass


def _safe_register_model(auto_cls, config_cls, model_cls):
    try:
...
_safe_register_config("vision", VisionEncoderConfig)
_safe_register_config("mlp_projector", MlpProjectorConfig)
_safe_register_config("deepseek_vl_v2", DeepseekVLV2Config)
_safe_register_model(AutoModelForCausalLM, DeepseekVLV2Config, DeepseekVLV2ForCausalLM)

# Also register with the image-text-to-text auto factory so that the
# real Hub checkpoints (which ship no ``auto_map``/remote code) can be loaded
# through ``AutoModelForImageTextToText`` used by the OpenVINO exporter for the
# ``image-text-to-text`` task.
try:
    from transformers import AutoModelForImageTextToText

    _safe_register_model(AutoModelForImageTextToText, DeepseekVLV2Config, DeepseekVLV2ForCausalLM)
except Exception:
    pass
