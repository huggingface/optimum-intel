...
def get_muse_glimmer_vision_inputs(grid_thw, vision_config, dtype=None):
    """Compute the grid-derived auxiliary inputs for the MuseGlimmer vision graph.

    The native vision stack builds bilinear position-embedding gathers, window
    reordering, rotary position ids, per-window attention masks and the pixel
    shuffle permutation from ``image_grid_thw`` using ops OpenVINO cannot trace
    (``meshgrid`` / ``index`` / ``.tolist()``). We precompute them here so both the
    dummy export inputs and the runtime feed them to the vision graph as tensors,
    which keeps the exported graph resolution-agnostic. Shared by the export dummy
    generator and the OpenVINO runtime so both stay in lockstep.
    """
    import torch
    ...
    if dtype is None:
        dtype = torch.float32
    grid_thw = grid_thw if isinstance(grid_thw, torch.Tensor) else torch.as_tensor(grid_thw)
    grid_thw = grid_thw.to(torch.long)

    merge_size = vision_config.merge_size
    patch_size = vision_config.patch_size
    num_grid_per_side = vision_config.pos_emb_height

    bilinear_indices, bilinear_weights = get_vision_bilinear_indices_and_weights(
        grid_thw, num_grid_per_side=num_grid_per_side, spatial_merge_size=1
    )
    bilinear_weights = bilinear_weights.to(dtype)

    window_index, cu_window_seqlens = get_vision_window_index(
        grid_thw,
        spatial_merge_size=1,
        window_size=vision_config.pos_emb_height * patch_size,
        patch_size=patch_size,
    )
    cu_window_seqlens = torch.unique_consecutive(torch.tensor(cu_window_seqlens, dtype=torch.int32))
    ...
    seq_len = int((grid_thw[:, 0] * grid_thw[:, 1] * grid_thw[:, 2]).sum())

    def _build_mask(cu):
        mask = torch.zeros((1, 1, seq_len, seq_len), dtype=torch.bool)
        for i in range(1, len(cu)):
            mask[..., cu[i - 1] : cu[i], cu[i - 1] : cu[i]] = True
        out = torch.zeros((1, 1, seq_len, seq_len), dtype=dtype)
        out.masked_fill_(torch.logical_not(mask), torch.finfo(dtype).min)
        return out

    attention_mask = _build_mask(cu_seqlens)
    window_attention_mask = _build_mask(cu_window_seqlens)

    ps_parts = []
    offset = 0
    for t, h, w in grid_thw.tolist():
        t, h, w = int(t), int(h), int(w)
        ds = torch.arange(h * w).view(h // merge_size, merge_size, w // merge_size, merge_size)
        ds = ds.permute(0, 2, 1, 3).reshape(-1)
        if t > 1:
            frame_offsets = (torch.arange(t) * h * w).view(t, 1)
            ds = (ds.unsqueeze(0) + frame_offsets).reshape(-1)
        ps_parts.append(ds + offset)
        offset += t * h * w
    pixel_shuffle_index = torch.cat(ps_parts)

    return {
        "bilinear_indices": bilinear_indices.to(torch.long),
        "bilinear_weights": bilinear_weights,
        "window_index": window_index.to(torch.long),
        "position_ids": position_ids.to(torch.long),
        "attention_mask": attention_mask,
        "window_attention_mask": window_attention_mask,
        "pixel_shuffle_index": pixel_shuffle_index.to(torch.long),
    }
