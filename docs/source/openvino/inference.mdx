...
By default, support dynamic shapes are supported, enabling inference for inputs of every shapes. To speed up inference, static shapes can be enabled by giving the desired inputs shapes wtih [.reshape()](reference#optimum.intel.OVBaseModel.reshape).
...
For models that handles images, you can also specify the `height` and `width` when reshaping your model:
