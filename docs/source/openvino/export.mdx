...
The model argument can either be the model ID of a model hosted on the [HF hub](https://huggingface.co/models) or a path to a model hosted locally.
...
You can also apply fp16, 8-bit or 4-bit weight compression on the Linear, Convolutional and Embedding layers when exporting your model by setting `--weight-format` to respectively `fp16`, `int8` or `int4`:
...
Once the model is exported, you can now [load your OpenVINO model](https://huggingface.co/docs/optimum/main/en/intel/inference)
