...
Optional arguments:
  ...
  --dataset DATASET     The dataset used for data-aware compression or quantization with NNCF. Can be a dataset name
                        (e.g., 'wikitext') or a string with options (e.g., 'wikitext2:seq_len=128'). Currently supported
                        option: seq_len. For language models you can use the one from the list
