...
Optional arguments:
  --task TASK           The task to export the model for. If not specified, the task will be auto-inferred based on
                        ...
                        'image-to-video', 'image-segmentation', 'inpainting', 'sentence-similarity', 'text-to-audio', 'image-to-text',
                        'automatic-speech-recognition', 'token-classification', 'text-to-image', 'text-to-video', 'audio-classification',
  ...
  --dataset DATASET     The dataset used for data-aware compression or quantization with NNCF. Can be a dataset name
                        ...
                        the dataset must be set to 'textvqa' ('contextual' is deprecated). Note: if none of the data-aware compression algorithms
