...
## [MiniCPM-V-4 Eagle3] (locally-trained)
The export also supports Eagle3 speculative-decoding draft models that reuse the
AngelSlim `Eagle3` draft architecture (`LlamaForCausalLMEagle3`) but are trained
outside the AngelSlim release, e.g. checkpoints targeting a VLM base model:
- MiniCPM-V-4_eagle3 (targets MiniCPM-V-4)
