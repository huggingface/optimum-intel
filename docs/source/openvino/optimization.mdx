...
Bilingual or large-vocabulary models (e.g. models pairing Arabic/English or other non-English
corpora with a >50k-token vocabulary) tend to be more sensitive to aggressive int4 compression of
the embedding/`lm_head` layers. For such models, a lower `ratio` combined with `quant_method="awq"`,
`scale_estimation=True` and `lora_correction=True` typically recovers accuracy at the cost of a
larger compressed model — see the `humain-ai/ALLaM-7B-Instruct-preview` entry in `configuration.py`
