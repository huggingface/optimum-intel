...
Whisper model can be quantized without the need for preparing a custom calibration dataset. Please see example below.

```python
ov_model = OVModelForSpeechSeq2Seq.from_pretrained("openai/whisper-large-v3-turbo", load_in_8bit=False)
ov_quantizer = OVQuantizer(ov_model)
