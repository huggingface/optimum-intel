...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td style="text-align: center; vertical-align: middle;">text-to-image<br>(OVStableDiffusionPipeline)</td>
            <td style="text-align: center; vertical-align: middle;">
                <button
...
model = OVModelForVisualCausalLM.from_pretrained(
    ...
    quantization_config=OVPipelineQuantizationConfig(
        pipeline_quantization_configs={
            "lm_model": OVWeightQuantizationConfig(bits=4, group_size=128, dataset="contextual", quant_method="awq"),
