...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>
                <button class="copy-btn" onclick="copyCommand(this, 'optimum-cli export openvino -m HuggingFaceH4/tiny-random-LlamaForCausalLM --weight-format int4 --group-size 4 ./save_dir')">+</button>
...
<script>
    function copyCommand(element, command) {
        navigator.clipboard.writeText(command).then(() => {
            let notification = element.nextElementSibling;
