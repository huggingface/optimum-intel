...
Apart from optimizing a model after training like post-training quantization above, `optimum.openvino` also provide optimization methods during training, namely Quantization-Aware Training (QAT) and Joint Pruning, Quantization and Distillation (JPQD).
...
# This is a workaround for current limitation (See note below)
# tokenizer call to always pad to the required length of ov_model
tokenizer.__class__.__call__ = partialmethod(tokenizer.__class__.__call__, padding="max_length", max_length=128. truncation=True)
...
> Known limitation: current OpenVINO model is saved/generated with input of fixed shape, meaning inference call to model is only functional when input matched the defined shape. See snippet above shows a workaround to pad output of tokeniker to match OVModel input shape. 