...
Currently, OpenVINO only supports static shapes when running inference on Intel's GPU. FP16 precision can also be enabled in order to further decrease latency.
...
Don't forget to save the resulting OpenVINO model using the `save_pretrained` method.
...
To further speedup inference, the model can be statically reshaped :
...
# Run inference with the defined shapes
