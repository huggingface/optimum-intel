...
When fixing the shapes with the `reshape()` method, inference cannot be performed with an input of a different shape. By modifying your pipeline, you can make sure to pad (or truncate) your inputs in order to perform inference with a static model.

```python
cls_pipe = pipeline(
    "text-classification", model=model, tokenizer=tokenizer, max_length=9, padding="max_length", truncation=True
)
result = cls_pipe("Hello")
```
