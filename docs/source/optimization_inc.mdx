...
# Save the PyTorch checkpoint, alongside with the ONNX model as `save_onnx_model` is set to True
...
model = INCQuantizedModelForSequenceClassification.from_pretrained(model_name)
```
