...
Intel Neural Compressor quantization can be applied on your model through the Optimum command-line.


You can easily add dynamic quantization on your model without chaning your original script.

For example, if you are running the [`run_glue.py`](https://github.com/huggingface/transformers/blob/v4.26-release/examples/pytorch/text-classification/run_glue.py) script from from HuggingFace Transformers.


You can simply replace the following command line:
